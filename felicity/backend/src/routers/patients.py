...
from schema.schemas import Patient, PatientCreate, Account
...
async def get_patients(
    db: Session = Depends(get_db), acct: Patient = Depends(_get_current_user)
) -> List[Patient]:
    if acct is not None:
...
async def update_account(
    ...
    if user is not None:
        await _update_patient(patient_id=patient_id, patient=patient, db=db)
