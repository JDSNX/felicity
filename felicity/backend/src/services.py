...
from models import Account as Account_Model, Patient as Patient_Model
...
async def authenticate_user(username: str, password: str, db: Session) -> Account_Model:
...
async def _account_selector(username: str, db: Session) -> Account_Model:
    acct = db.query(Account_Model).filter(Account_Model.username == username).first()

    if acct is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Account does not exists."
        )

    return acct


async def _patient_selector(patient_id: int, db: Session) -> Patient_Model:
    acct = db.query(Patient_Model).filter(Patient_Model.id == patient_id).first()

    if acct is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Patient does not exists."
        )

    return acct


async def delete_account(username: str, db: Session) -> None:
    stud = await _account_selector(username=username, db=db)
    db.delete(stud)
    db.commit()


async def update_account(username: str, account: AccountUpdate, db: Session) -> Account:
    acc_db = await _account_selector(username=username, db=db)
    account.password = hash.bcrypt.hash(account.password)
...
async def add_patient(patient: PatientCreate, db: Session) -> Patient_Model:
...
async def get_patient(patient_id: int, db: Session) -> Patient:
    return await _patient_selector(patient_id=patient_id, db=db)


async def delete_patient(patient_id: str, db: Session) -> None:
    patient = await _patient_selector(patient_id=patient_id, db=db)
    db.delete(patient)
    db.commit()


async def update_patient(
    patient_id: int, patient: PatientCreate, db: Session
) -> Patient:
    patient_db = await _patient_selector(patient_id=patient_id, db=db)
