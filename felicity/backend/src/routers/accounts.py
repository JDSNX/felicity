...
from schema.schemas import Account, AccountCreate, Token, AccountUpdate
...
async def create_account(
    account: AccountCreate, db: Session = Depends(get_db)
) -> Token:
...
async def get_accounts(
    db: Session = Depends(get_db), acct: Account = Depends(_get_current_user)
) -> List[Account]:
    if acct is not None:
...
async def update_account(
    ...
    if user is not None:
        await _update_account(username=username, account=account, db=db)
